...
public class ReadEmails {
	...
	private static final Logger LOGGER = LogManager.getLogger(ReadEmails.class);
	...
	public void connectionAccountMail() throws WebException {
		...
		if ((server == null) || (server.equals(""))) {
			msg = "Informe endere��o IMAP do servidor!";
			LOGGER.error(msg);
			throw new WebException(msg);
		}
		...
		try {
			...
			/*
			 * Como h� necessidade de autentica��o � criada uma autentica��o que
			...
			store.close();
			if (!deleteMessages) {
				LOGGER.info("--------------Finaliza��o do Processo de Leitura de Emails-----------------");
			} else {
				LOGGER.info("--------------Finaliza��o do Processo de Deletar de Emails-----------------");
			}
