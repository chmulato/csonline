...
public class ChangePasswordController extends AbstractController implements Serializable {
	...
	private void loadSessao() {
		...
		LOGGER.info("Carregando controle da p�gina de usu�rio ...");
		try {
	...
	public String save() {
		...
		try {
			...
			if ((getId() == null) || getId().equals(new Integer(0))) {
				throw new WebException("Informe id do usu�rio logado!");
			}
			...
			if ((!getRole().equals("ADMINISTRATOR")) && (!getRole().equals("BUSINESS"))
					&& (!getRole().equals("CUSTOMER")) &&
