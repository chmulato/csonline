...
public class DeliveriesController extends AbstractController implements Serializable {
	...
	private static final Logger LOGGER = LogManager.getLogger(DeliveriesController.class);
			...
			if (loginController.isLogged()) {
				...
				if (profile.equals("BUSINESS")) {
					...
					results = FactoryService.getInstancia().getDeliveryService()
							.listAllDeliveryBusinessNotCompleted(idBusiness);
					...
					results = FactoryService.getInstancia().getDeliveryService()
							.listAllDeliveryCustomerNotCompleted(idCustomer);
					...
					results = FactoryService.getInstancia().getDeliveryService()
							.listAllDeliveryCourierNotCompleted(idCourier);
	...
	public String delete() {
		...
		try {
			...
			if ((parameter != null) && (!parameter.equals(""))) {
				...
				throw new WebException("Parâmetro vazio! Informe o id da entrega.");
			}
