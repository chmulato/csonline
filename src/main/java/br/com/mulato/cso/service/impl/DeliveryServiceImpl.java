...
public class DeliveryServiceImpl
	...
	private void sendMessageFromBusinessDelivery(final Integer deliveryId) throws WebException {
		...
		if (delivery.getBusiness() == null) {
			throw new WebException("Negócio da entrega n�o encontrado.");
		}
		...
		if ((delivery.getReceived()) && (!delivery.getCompleted())) {
			...
			message.append("Endere�o base:		    ").append(delivery.getStart()).append("\r\n");
			message.append("Endere�o de entrega:    ").append(delivery.getDestination()).append("\r\n");
			message.append("Dist�ncia: 	 			").append(delivery.getKm()).append(" Km\r\n");
			message.append("Contato: 	 			").append(delivery.getContact()).append("\r\n");
			message.append("Descri��o: 	 			").append(delivery.getDescription()).append("\r\n");
	...
	private void sendMessageFromCustomerDelivery(final Integer deliveryId) throws WebException {
		...
		final String subject = "Agendar corrida No. " + delivery.getId()
				+ ". Por favor, encaminhe um entregador e confirme valor!";
		...
		message.append("Contato: 	 			").append(delivery.getContact()).append("\r\n");
		message.append("Descri��o: 	 			").append(delivery.getDescription()).append("\r\n");
		message.append("Adicional: 	 			").append(delivery.getAdditionalCost())
				.append(" reais (Valor a confirmar)\r\n");
	...
	public List<DeliveryVO> listAllDeliveryBusinessNotCompleted(final Integer idBusiness) throws WebException {
		...
		LOGGER.info("Listar todas as entregas n�o completadas do negócio.");
	...
	@Override
	public void saveDeliveryByBusiness(final DeliveryVO delivery, final boolean send_mail, final boolean send_sms)
			throws WebException {
		...
		try {
			...
			if (delivery.getId() == null) {
				...
				if (send_sms) {
					...
					final CourierVO courier = FactoryDAO.getInstancia().getCourierDAO()
							.find(delivery.getCourier().getId(), false);
	...
	public void saveDeliveryByCustomer(final DeliveryVO delivery, final boolean send_mail) throws WebException {
		...
		if (delivery.getDescription() == null) {
			throw new WebException("Informe descri��o da entrega!");
		}
